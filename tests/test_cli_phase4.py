...
''')
    ...
    nested_source_dir = tmp_path / "pkg"
    nested_source_dir.mkdir()
    nested_file = nested_source_dir / "nested_module.py"
...
''')
    ...
    ignored_test_dir = tmp_path / "tests"
    ignored_test_dir.mkdir()
    ignored_file = ignored_test_dir / "test_fixture.py"
...
class TestReviewCodeCommand:
    ...
    def test_review_code_all_flag(self, runner, temp_project):
        ...
        try:
            ...
            # 应该递归审查源码文件，并跳过测试目录
            assert '审查' in result.output
            assert 'nested_module.py' in result.output
            assert 'test_fixture.py' not in result.output
        finally:
            os.chdir(old_cwd)
